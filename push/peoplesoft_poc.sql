...
WITH 
  wk_silvercte AS (
    ...
    FROM
        "Peoplesoft_Silver".ps_personal_data
  ),
  wk_addresses AS (
    SELECT DISTINCT 
        "emplid" AS employee_id,
    ...
    FROM
        "Peoplesoft_Silver".ps_addresses
  ...
  wk_divers_ethnic AS (
    SELECT DISTINCT 
        "emplid" AS employee_id
    FROM
        "Peoplesoft_Silver".ps_divers_ethnic
  ),
  wk_ps_visa_pmt_data AS (
    ...
    FROM
        "Peoplesoft_Silver".ps_visa_pmt_data
  ),
  wk_pers_nid AS (
    SELECT DISTINCT 
        "emplid" AS employee_id,
  ...
  wk_drivers_lic AS (
    ...
    FROM
        "Peoplesoft_Silver".ps_drivers_lic
  ),
  wk_employment AS (
    SELECT DISTINCT 
        "emplid" AS employee_id,
    ...
    FROM
        "Peoplesoft_Silver".ps_employment
  )
--- Combine all CTEs
SELECT DISTINCT
  s.employee_id, 
